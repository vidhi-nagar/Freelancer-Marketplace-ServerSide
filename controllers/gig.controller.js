...
export const updateGig = async (req, res, next) => {
  try {
    const gig = await Gig.findById(req.params.id);
    if (!gig) return next(createError(404, "Gig not found!"));
    if (gig.userId !== req.userId) return next(createError(403, "You can update only your gig!"));

    const updatedGig = await Gig.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true });
...
export const getGigs = async (req, res, next) => {
  ...
  try {
    const gigs = await Gig.find(filters).sort({ [q.sort || "sales"]: -1 });
